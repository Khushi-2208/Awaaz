...
export default function Page() {
  ...
  const detectLanguage = (text: string): string => {
    ...
    const bhojpuriKeywords = ['के', 'सकेला', 'बा', 'बानी', 'रहल', 'जाला', 'आवेला', 'करेला', 'खातिर', 'रउआ'];
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      const detectedLang = detectLanguage(inputText);
      // Use hi-IN for both Hindi and Bhojpuri since they use the same script
      recognitionRef.current.lang = (detectedLang === 'hi' || detectedLang === 'bho') ? 'hi-IN' : 'en-IN';
      recognitionRef.current.start();
      setIsListening(true);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Received schemes:', data.schemes?.length || 0);
      setSchemes(data.schemes || []);
    } catch (error) {
      console.error('Error fetching schemes:', error);
      let errorMsg = '';
      if (detectedLang === 'bho') {
        errorMsg = 'माफ करीं, कुछ गलती हो गइल। फेर से कोशिश करीं।';
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 border border-gray-100">
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <input
                ...
                placeholder="Tell me about yourself... / अपने बारे में बताएं... / अपना बारे में बताईं..."
                className="flex-1 text-gray-900 px-5 md:px-6 py-3 md:py-4 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-blue-500 text-base md:text-lg transition-all"
              />
            ...
            <p className="text-center text-xs md:text-sm text-gray-500 mt-4">
              🎤 Click mic to speak or type directly
              <br />
              माइक पर क्लिक करें या सीधे टाइप करें • माइक पर क्लिक करीं भा सीधा टाइप करीं
              <br />
              <span className="text-blue-600 font-medium">💡 Supported: English, Hindi (हिंदी), Bhojpuri (भोजपुरी)</span>
            </p>
          </div>
        </div>
                  ...
                  >
                        ...
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <button
                            onClick={() => {
                              ...
                              let eligibilityLabel, benefitsLabel;
                              ...
                              speakText(
                                `${scheme.name}। ${eligibilityLabel}: ${scheme.eligibility}। ${benefitsLabel}: ${scheme.benefits}`,
                                scheme.id
                              );
                            }}
                            className={`p-3 rounded-full transition-all ${
                    ...
                    <div className="p-6">
                      {scheme.eligibility && (
                        <div className="mb-5">
                          <h5 className={`font-bold text-gray-900 mb-2 flex items-center gap-2 ${colors.text}`}>
                            <div className={`w-2 h-2 bg-gradient-to-r ${colors.bg} rounded-full`}></div>
                            Eligibility / पात्रता
                          </h5>
                          <p className="text-gray-700 text-sm leading-relaxed">{scheme.eligibility}</p>
                        </div>
        ...
        {!isLoading && schemes.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
              ...
              <br />
              🇬🇧 "I am 30 years old male from Bihar and want health schemes"
              <br />
              🇮🇳 "मैं बिहार का 25 साल का किसान हूं"
              <br />
              🗣️ "हम बिहार से बानी और हमके रोजगार योजना चाही"
            </p>
          </div>
        )}
      </div>
